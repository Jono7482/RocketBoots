...
pygame.display.set_icon(load(get_data("icon.png")).convert_alpha())
...
class Display(object):
    def __init__(self):
        self.background = load(get_data("skynb.jpg")).convert()
        self.bgpause = load(get_data("bgpaused.png")).convert_alpha()
...
class DudeObj(object):
    def __init__(self):
        self.dudeimg = load(get_data("littledude1.png")).convert_alpha()
        self.dudeRect = pygame.Rect(150, resolution[1] / 3, 27, 30)
        self.flame_list1 = [load(get_data("flame1.png")).convert_alpha(),
                            load(get_data("flame2.png")).convert_alpha()]
        self.flame_list2 = [load(get_data("flame3.png")).convert_alpha(),
                            load(get_data("flame4.png")).convert_alpha()]
...
def collision():
    ...
    if gameState == "playing":
        ...
        if dude.dudeRect.collidelist(rect_list) == -1:
            ...
            explosion(dude.dudeRect.center)
            clock.tick(1)
            death()
...
class LevelObj(object):
    def __init__(self):
        ...
        self.gaptop = 0
        self.this_gap = 0
        self.image = load(get_data("obstacle2.png")).convert()
        self.tick = 500
        self.obstacle_list = []
...
def create_hills():
    hills_list = [load(get_data("hills1.png")).convert_alpha(),
                  load(get_data("hills2.png")).convert_alpha()]
...
def create_clouds():
    clouds_array = [load(get_data("cloud1.png")).convert_alpha(),
                    load(get_data("cloud2.png")).convert_alpha(),
                    load(get_data("cloud3.png")).convert_alpha(),
                    load(get_data("cloud4.png")).convert_alpha()]
...
def make_text_objs(location, text, font_size=30, rgb=black, shadow=None, pos="xy"):
    font = pygame.font.Font(get_data("freesansbold.ttf"), font_size)
    x, y = location
...
class AudioHandler(object):
    def __init__(self):
        ...
        self.music.load(get_data("Indiekid.mp3"))
        self.boots1wav = pygame.mixer.Sound(get_data("boots1.wav"))
        self.explosion1wav = pygame.mixer.Sound(get_data("explosion1.wav"))
        ...
        self.beep1wav = pygame.mixer.Sound(get_data("beep1.wav"))
        self.beep2wav = pygame.mixer.Sound(get_data("beep2.wav"))
...
class ScoreBoard(object):
    ...
    def load_scoreboard(self):
        self.scoreboard = pickle.load(open(get_data("scoreboard.p"), "rb"))

    def dump_scoreboard(self):
        data = self.scoreboard
        try:
            pickle.dump(data, open(get_data("scoreboard.p"), "wb"), protocol=2)
        except OSError:
